...
todo 
==== 
- bug: 0️⃣ I was running v.13 and the script asks to update to v1.17. 
					It then say V1.17 has been installed and I then close nd restrat the program but it does the same thing again. I can manually download v1.17 but I wonder why it is suddenly having this problem (or did I forget something?)
					****** if script folder on icloud? download delay? ask @ihf
									when pgm says please restart, in same tab? too quick?
					- close this tab 
					- load new
					- edit script
					- notification in some seconds to relaunch Pythonista
						you will be notified to restart automatically the new version
					- abort
...
try:
	...
	if new_version > Version:
		...
		if b == 1:
			...
			url = 'https://raw.githubusercontent.com/cvpe/Pythonista-scripts/master/Gists/outline2.py'
			data = requests.get(url).content
			with open('outline.py', mode='wb') as out_file:
				out_file.write(data)
			del data
			console.alert(f"New version {new_version} has been installed\n\n.py\n.versions", 'Pythonista will now abort\nPlease restart it.', 'ok', hide_cancel_button = True)
			os.abort()
except Exception as e:
	#print('error',e)
	pass
...
class Outliner(ui.View):
	...
	def init2(self):
		mytrace(inspect.stack())
		# code only to be sure keyboard height will be knwon at befin of program: begin
		tf = ui.TextField()
		tf.frame = (0,0,10,10)
		self.add_subview(tf)
		tf.begin_editing()
	...
	def textview_did_begin_editing(self, textview):
		...
		# don't me why but without this line, on iPhone in portrait mode only, 
