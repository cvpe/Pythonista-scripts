...
- .. bug: after saving an outline closing program, and then restarting it, 
					I got the following error:
					Traceback (most recent call last):
				  File "/private/var/mobile/Containers/Shared/AppGroup/05D14C00-CCAC-4E98-A3CC-96C6944B8534/Pythonista3/Documents/outline.py", line 3205, in <module>
						main()
					File "/private/var/mobile/Containers/Shared/AppGroup/05D14C00-CCAC-4E98-A3CC-96C6944B8534/Pythonista3/Documents/outline.py", line 3201, in main
						mv = Outliner()
					File "/private/var/mobile/Containers/Shared/AppGroup/05D14C00-CCAC-4E98-A3CC-96C6944B8534/Pythonista3/Documents/outline.py", line 1104, in __init__
...
- .. new: Keep a creation date/time and change date/time,and due date/time 
					for each outline item and the ability to sort Alphabetically, on change , creation , or due date.
					
- .. new: Ability to show/hide items with a checked check box. 
					Not sure about sorting on checked items.
					
- .. new: force a Pythonista iCloud file to be downloaded.py

					workaround: use Files app for first downloading file
					
					- ok menu open file
					- ok menu open log
					- not ok arg open file
					- save
					- pick folder
					- arg = ext -> uti
					- check if we load an external file that we would be allowed to save it
					
						- file_pickerwithout unloaded files
						- dialogs.pick_document without local Pythonista files
						- first screen with choice local Pythonista, Files app
						- local Pythonista: files_picker documents
						- files app: dialogs.pick_document
						- new: files_picker folder
						- see MyPickDocument uti .folder
						- file_picker only for files, or no more...
						- extension -> uti
							UniformTypeIdentifiers 
							let fileExtension = "html"
							let typeIdentifier = UTType(filenameExtension: fileExtension)
						- when save, if not PytHonista, use SaveInFiles
						
- .. ask: if arg = .txt, show only .txt or all others also but grayed

- .. new: add image (not prioritary)

					questions: This program could support images, that's sure, but images could not be integrated in the text file, so you would not be able to print the text with images by normal printing tools. So, I would need to store them in the .content file, and the printing of text with its integrated images could only be done by this program... And that's a big modification. Assume we support images, we could start the process by an additional option in the popup menu. This option could allow to pick an image file. But do you want images as separate outline row or do you want an image integrated in the text of a row? How do you want to specifiy the size of the image in the text?
			
					- bug: ❗️iphone, choice local/files directly returns with None❗️
					- bug: typing multiple soft lines do not resize textview
					- ok for program but quid in .txt if want to print
...
- .. new: multiple level undo
					- bug: tab => bad undo successifs
					- action = text or left delete, save only (row,row data)?
...
class Outliner(ui.View):
	...
	def tableview_did_select(self, tableview, section, row):
		...
		if tableview.name != 'outliner':
			...
			if self.device_model == 'iPad':
				...
				elif tableview.name.startswith('from'):					
					folder = tableview.data_source.items[self.selected_row[1]]
					self.remove_subview(tableview)
					del tableview
					self.shield.hidden = True
					self.folder_process(folder)
	...
	def files_action(self, sender, act):
		...
		if act == 'New':
			...
			if sender == 'Open':
				...
				#f = self.pick_file('from where to open the file', self['b_files'])
			...
			if not self.file:
				txt_dir = File_Picker.file_picker_dialog('Select a folder where to create the new', root_dir=self.path_to_name, select_dirs=True, only=True)
				if not txt_dir:
			...
			if not self.file:
				...
				return
			#f = self.pick_file('from where to open the log', self['b_files'])
			f = File_Picker.file_picker_dialog('Pick a log file', root_dir=self.path_to_name)
			if not f:
				console.alert('❌ No file has been picked','','ok', hide_cancel_button=True)
				return
			self.tv.text = ''
	...
	def pick_file(self, title, sender, ext=None, uti=['public.item'], callback=None):
		x = sender.x + sender.width/2
		y = sender.y + sender.height
		x,y = ui.convert_point(point=(x,y), from_view=sender, to_view=None)
		sub_menu = ['Local Pythonista', 'Files app']
		tv = ui.TableView(name=title)
		tv.row_height = 30
		h = tv.row_height*len(sub_menu)
		tv.frame = (0,0,250,h)
		tv.data_source = ui.ListDataSource(items=sub_menu)
		...
		tv.delegate = self
		self.selected_row = None
		self.parameters = (title,ext,uti,callback)
		self.present_popover(tv, 'popover',popover_location=(x,y),hide_title_bar=False)
		#print('pick_file',self.selected_row)
		if self.device_model == 'iPad':
			if not self.selected_row:
				return
			folder = sub_menu[self.selected_row[1]]
			self.folder_process(folder)
		else:
			# iPhone
			# process done in tableview_did_select
			return 'delayed'

	def folder_process(self,folder):
		title,ext,uti,callback = self.parameters
		if folder == 'Local Pythonista':
			path = os.path.expanduser('~/Documents/')
			if ext:
				fil = File_Picker.file_picker_dialog('Pick a text file', root_dir=self.path_to_name, file_pattern=r'^.*\{}$'.format(ext))	
			else:			
			...
			UIDocumentPickerViewController = self.MyPickDocument(600,500, callback=callback, UTIarray=uti)
			return 'delayed'
								
	@on_main_thread	
	def MyPickDocument(self, w, h, callback=None, UTIarray=['public.item'], PickerMode=1):
	...
	def popup_menu_action(self,sender, row=None):
		...
		if act == 'hide children':
			...
			self.image_row = row
			f = self.pick_file('from where to open the image', self.cells[row].content_view['outline'], uti=['public.image'], callback=self.pick_image_callback)
			if not f:
			...
			#f = File_Picker.file_picker_dialog('Pick an image file', root_dir=self.path_to_name)
	...
	def button_show_action(self,sender):
			x = sender.x + sender.width/2
			y = sender.y + sender.height
			sub_menu = ['Collapse all','Expand all']
