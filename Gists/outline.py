...
todo 
====
- new: 0️⃣ understand name 2 and delete it
- new: 0️⃣ check if better way to check if .content downloaded
- new: 0️⃣ Full photo? When and where?
					Ok for small, medium, large but to which number of rows they do correspond?
					Ok for default: small, medium or large?
					Cancel possible via cancel the pick window (top left button ) but I could add a button cancel below photos
...
- bug: 1️⃣ tab => bad successive undo's'
...
class Outliner(ui.View):
	...
	def pick_file(self, title, ext=None, uti=['public.item'], callback=None, ask=True, init=None):
		...
		if ask:
			# ask user from where
			if uti == ['public.image']:
				b = console.alert(title,'','Local Pythonista', 'Files app', 'Photos', hide_cancel_button=True)
			else:
