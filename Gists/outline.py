...
- bug: 0️⃣ sort dates
					- If my outline is collapsed and I select view by completion date, 
						and then select sort button to change to sort down, the display gets messed up in that lines are overlapping.
						comes from tableview_height_for_section_row needs to check sort type
					- rows height not correct, see it in bottom line of each row
	...
	def show_action(self,act):
		...
		if self.show_mode == 'View dates':
			...
			self.dates_sort_mode = 0 	# set before reload because tableview_cell_for_row
																# uses it
			self.tv.y = self.ht + self['dates_title'].height
			self.tv.reload_data()
