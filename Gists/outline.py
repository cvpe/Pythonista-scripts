...
- .. log
			- setting on/off
			- on: textview_should_change: log replacement, range + lf
			- on: log move, see details
			- file xxxx.log
			- play it
...
- .. - if changing font size or font generates longer/shorter lines, redo
- .. - outline attributes, after generation of outlines
				loop on lines
					remove previous soft lf
					if line too large
						generate soft lf (text=lf but content = special lf)
						content:
							if previous line no tuplé normal lf
							else outline identique to previous
...
class Outliner(ui.View):
	...
	def drop(self,found):
		#print('drop:', found, len(self.tv.text))
