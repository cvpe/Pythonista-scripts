...
todo 
====
- bug: 0️⃣ self.auto_save seems to be not functional
- new: 1️⃣ generate rtf ask @ihf?
					- font, color of screen
					- ask type txt, rtf
- new: 1️⃣ support level 0 = [] > no outline
					- ⁉️doubletap on checkbox to get popup menu		
					if self.outline_formats[self.outline_format][0][0] == '':
						# format with 1st level without outline
						# use ..... on ....... for popup
						....(b,self.single_or_double_tap_handler)				
- new: 2️⃣ 2nd arg as a path to open outline from other path than current
					should work for arg1 = file 
											for ext 
- bug: 3️⃣ undo: bad successive 🛑 
- new: 🔟 print text+images: print option in files submenu		
- new: 🔟 font/bold/italic/color per level
					- default for all files in .prm, same dict as in .content
					- but if different in one file, store in .content
						- {0:{'font':name, ....., 'bold':True, 'color':'red'}}
	...
	def files_action(self, sender, act):
		...
		if act == 'New':
			...
			if not self.file:
				...
				return
			b = 1#console.alert('Share as','', '.txt','.rtf','cancel', hide_cancel_button=True)
			if b == 3:
				return
			path = sys.argv[0]
			i = path.rfind('/')
...
\\fs24
'''
					t = t.replace('OUTLINE_COLOR','\\red'+str(int(255*self.outline_rgb[0])) + '\\green'+str(int(255*self.outline_rgb[1])) + '\\blue'+str(int(255*self.outline_rgb[2])))
				for item in self.tv.data_source.items:
					...
					for l in ls:
						if first:
							if path[-3:] == 'txt':
								...
								t += "{\\pard {\\cf1 "+outline+"}{\\cf2 "+l+"}\\par}"
							first = False
						else: 
							if path[-3:] == 'txt':
								t += ' '*len(outline) + l + lf #    Word 2
							elif path[-3:] == 'rtf':
								t += "{\\pard {\\cf1 "+" "*len(outline)+"}{\\cf2 "+l+"}\\par}"		
				if path[-3:] == 'rtf':		
					t += "}"
				fil.write(t)
				del t
