...
todo 
==== 
- new: 0️⃣ automatic check update or update or via menu 
					- check internet, error => message
					- if ok, get versions file on github
					- if different of version of script
						- warn user
						- check if .py accessible
						- warn user, ask if automatic update
						- if yes, get and install both files
						- ask user to restart
- bug: 0️⃣ - rename file => delete its event and create a new one
					- no more use of eventIdentifier for due date event
