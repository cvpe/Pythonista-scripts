...
This program could support images, that's sure, but images could not be integrated
in the text file, so you would not be able to print the text with images by normal
printing tools. 
So, I would need to store them in the .content file, and the printing of text
with its integrated images could only be done by this program...
And that's a big modification.
Assume we support images, we could start the process by an additional option in
the popup menu. This option could allow to pick an image file. 
But do you want images as separate outline row or do you want an image integrated in the text of a row?
...
class Outliner(ui.View):
	...
	def present_popover(self, view, mode, popover_location=None, hide_title_bar=False, force=False):
	...
	def files_action(self, sender, act):
		...
		if act == 'New':
			...
			if sender == 'Open':
				...
				#f = self.pick_file('Pick a text file')
			...
			if not self.file:
				...
				return
			#f = self.pick_file('Pick a log file', root_dir=self.path_to_name)
	...
	def keyboard_frame_did_change(self, frame):
		...
		self.tv.content_inset = (0,0,frame[3],0)
