...
todo 
====
- .. new: enable/disable the checkbox on selected portions of the outline
					at least at a selected level with child nodes
...
- .. try: redisp visible rows instead of reload_data except if del/add/move
...
class Outliner(ui.View):
	...
	def button_filter_action(self,sender):
		fields = []
		fields.append({'title':'filter type', 'type':'text', 'value':'>', 'key':'segmented1', 'segments':['<', '=', '>']})
		fields.append({'title':'filter level', 'type':'number', 'pad':pad_integer, 'value':'0'})
	...
	def button_files_action(self,sender):	
		if not isinstance(sender, ui.ButtonItem):
			act = sender
		else:
