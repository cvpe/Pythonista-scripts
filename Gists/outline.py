...
- bug: 0️⃣ got an IndexError when I went to menu and did Delete with Children.
- bug: 0️⃣ if I go to Settings or Show only if Level, and touch Done, I get an AttributeError (no attribute close).
...
class Outliner(ui.View):
	...
	def paste(self, found, paste_items):
		mytrace(inspect.stack())
		#print('paste:', found)
		self.undo_save('past')
		...
		row = 0
		pasted_items = []
		while row <= (len(paste_items)-1):
			vals,n,opts = paste_items[row]['content']
			if first:
				vals1 = vals.copy()
				if under_outline:
					# drop under outline
			...
			n = len(outline)
			item = {}
			item['title'] = paste_items[row]['title']
			item['outline'] = outline
			if 'dates' in opts:
	...
	def popup_menu_action(self,actin, row=None):
		...
		if act == 'hide children':
			...
			self.paste(row, self.paste_items)
		elif 'paste_text ' in act:	
			self.paste(row, self.paste_text_items)
		elif act == 'add before':
			self.undo_save('CR')		
	...
	def single_or_double_tap_handler(self,data):
		...
		try:
			...
			ok = False
		if ok:
			sub_menu.append('paste '+t[:20]+'...')
			
		# check if paste text would be proposed
		ok = True
		try:
			clipboard_str = clipboard.get()
			clipboard_lines = clipboard_str.split(lf)
			self.paste_text_items = []
			for i in range(len(clipboard_lines)):
				item = {}
				item['title'] = clipboard_lines[i]
				item['content'] = ([i],1,{})
				self.paste_text_items.append(item)
			del clipboard_str
			if not isinstance(self.paste_text_items[0], dict):
				ok = False
			else:
				item = self.paste_text_items[0]
				t = item['title']
		except:
			ok = False
		if ok:
			sub_menu.append('paste_text '+t[:20]+'...')
	...
	def file_save(self, save_prm=True):
		...
		# store outline text as event at 01/01/2042 01:01
		...
		date1 = dateFormat.dateFromString_('20420101 01:01:00') 
		date2 = dateFormat.dateFromString_('20420101 01:01:01') 	
