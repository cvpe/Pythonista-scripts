...
- .. bug: 0️⃣ If I position the cursor at the end of 2.and hit return, 
					I get a new item at 3.0. However 2.1 and 2.2 have moved to be 3.1 and 3.2 which is not what should happen.
					...
					Insert a row but after children...*********
- .. new: 2️⃣ would be nice if the script only showed you outline files
						which extension? 
						solution: file xxx.yyy => xxx.yyy.content
											select .content and the script extract the xxx.yyy
											file_picker: extension .content
											uipicker: public.?
- .. bug: 1️⃣ pick folder to save and asking name does not appear if Files app (loop)
							- use when save
...
class Outliner(ui.View):
	...
	def pick_open_callback(self,param):
		...
		if not os.path.exists(self.path+self.file_content):
			...
			if '/private/var/mobile/Library/Mobile Documents/' in self.path:
				console.hud_alert('❌ '+self.file_content+' does not (yet) exist, retry', 'error', 3)
			else:
				console.hud_alert('❌ '+self.file_content+' does not exist', 'error', 3)
			return
