...
- .. new: add image (not prioritary)
					...
					- file_picker jpg, png, ...
					- size
					- outline row with image or include in text?
...
class Outliner(ui.View):
	...
	def tableview_height_for_section_row(self,tv,section,row):
		...
		#ho = ui.measure_string(tv.data_source.items[row]['title'], font=ft, max_width=self.width-x)[1]
		l = ui.TextView()
		...
		if 'hidden' not in opts and 'image' in opts:
			ho = opts['image'][1] # (image file name, h)
		...
		if tableview.name != 'outliner':
			...
			if self.device_model == 'iPad':
				...
				if tableview.name == 'files':
					...
					loc_format = tableview.data_source.items[self.selected_row[1]]
					self.remove_subview(tableview)
					del tableview
					self.shield.hidden = True
					self.show_action(loc_format)		
	...
	def pick_file(self, title, sender, ext=None):
		x = sender.x + sender.width/2
		y = sender.y + sender.height
		...
		if not self.selected_row:
			...
			print(fil)
			return fil
		elif self.selected_row[1] == 1:	
			UTI_array = ['public.text']	
			fil = dialogs.pick_document(types=UTI_array)			
			print(fil)
			return fil
	...
	def tableview_cell_for_row(self, tableview, section, row):
		...
		bb.size_to_fit()
		ho = bb.height
		bb.frame = (x,0,self.width-x-4,ho)
		...
		if not hidden:	
			if 'image' in opts:
				...
				iv.image = ui.Image.named(opts['image'][0])
				wi,hi = iv.image.size
				w = wi * hrow/hi
				iv.width = w
	...
	def button_show_action(self,sender):
			x = sender.x + sender.width/2
			y = sender.y + sender.height
			sub_menu = ['Show all', 'Collapse all','Expand all']
