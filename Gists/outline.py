...
class Outliner(ui.View):
	...
	def pick_open_callback(self,param, simul_find=None):
		...
		if prms.startswith('{'):
			...
			if '1st level has outline' in prms:
				...
				self.first_level_has_outline = True
			if 'applewatch_event' in prms:
				self.applewatch_event = prms['applewatch_event']
			else:
				self.applewatch_event = None
	...
	def file_save(self, save_prm=True):
		...
		add = True
		if self.applewatch_event:
			event = self.store.eventWithIdentifier_(ns(self.applewatch_event))
			if event:
				add = False
		if add:
			event = ObjCClass('EKEvent').eventWithEventStore_(self.store)
			...
			ret = self.store.saveEvent_span_error_(event, span, None)
			if ret:
				self.applewatch_event = str(event.eventIdentifier())
		...
		with open(self.path + new_filename + '.outline', mode='wt', encoding='utf-8', errors="surrogateescape") as fil:
			...
			prms['applewatch_event'] = self.applewatch_event
			t = str(c) + lf + str(prms)
			fil.write(t)
			del t
			del c
