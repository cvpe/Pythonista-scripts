...
Since I play with ui.TextView (ObjectiveC) inset to solve problem of desynchronization between text lines and their checkbox and hide/show buttons, a bad scroll is done when typing. This problem is still present, sorry for that.
And I'm looking for the reason for hours... 😢

- .. 0️⃣bug: type => scroll weird when typing, even without setcursor
 
- .. bug: backtab until no outline, next lines have bad renumbering
					prohibit backtab if target is no outline and there are outline after
					
- .. think about a better way to process outlines
			- store an array of outlines with their info, thus no more content+xxx+content
			- store these infos as an array in .content
			- memo when inserting/deleting a line, easier
			content = [([0,3,4],n,{options}),...]
	
- .. 0️⃣do not rebuild outlines when normal character typed
...
- .. undo a drop or other action(top menu button, save t/c before, reset)
		 use .copy to be sure when saving text and content
	
- .. .log with new or open file name +.log

- .. external keyboard tab and shift tab key	

- .. delete line with or without children
			- options de popumenu
...
class Outliner(ui.View):
	...
	def set_font(self,font_type=None,font_size=None,font_hidden_size=None):
		global font, font_hidden
		if font_type:
			self.font = font_type
	...
	def textview_should_change(self, textview, range_c, replacement, play=False):
		...
		if replacement == '\x01':
			...
			# =====================
			for ch in [lf, '\x01', '\x02', '\x03']:
				# check if not paste of a string containing special chatacters
				if ch in replacement:
					console.alert('❌ paste of CR, tab, back tab','not allowed','ok', hide_cancel_button=True)
					return False	# not allowed
	...
	def checkbox_button_action(self,sender):
		...
		self.content[i] = (self.content[i][0],self.content[i][1],opts)
		self.modif = True
	...
	def setCursor(self,i):
		...
		y = rect.origin.y
		h = rect.size.height
		#print(y,self.tv.content_size[1]-self.tv.height)
		y = max(0,min(y,self.tv.content_size[1]-self.tv.height))

		#print(i,y)
		if y < self.tv.content_offset[1] or y > (self.tv.content_offset[1] + self.tv.height - h):
