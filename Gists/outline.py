...
todo 
====
- ok 0️⃣bug: full review for renumbering for tab
- ok 0️⃣bug: full review for renumbering for lf
- .. 0️⃣bug: full review for renumbering for backtab
- ok 0️⃣bug: full review for renumbering for insert dropped 
- ok 0️⃣bug: full review for renumbering for delete original of dropped

- .. 0️⃣bug: drop after all lines does not do anything
- .. 0️⃣bug: backtab should not be allowed if generates level difference of 2 vs next

- .. delete line with or without children
			- options de popumenu
			- afficher zone original a detruire + confirmation?
			- del comme drop del part
			- renumbering after del area, comme drip del part
		...
		while k >= 0:
			if k > (len(self.content)-1):
				...
				if typ == 'tab':
					...
					if vals[:len(vals_before)] == vals_before:
						# child of tabbed outline
						nvals = vals_after + vals[len(vals_after):]
