...
todo 
====
- .. bug: here is an interesting bug. I figured I would create a new outline 
					and in order to make it easy, I opened a split screen on the iPad with the text on one side and a notepad with the text on the other. However, when I type the text on the Pythonista side, a new outline item is started whenever I get to the RHS of the screen
...
- .. new: access local, icloud in the same File_Picker
					- add all accessible local/icloud/icloud drive in File and Folder Picker
					- remove this setting
					- remove automatically from .prm
					- or if change, update also relative setting and prm
- .. new: support rotation of device while running
...
def my_form_dialog(title='', fields=None, sections=None, done_button_title='Done', wd=500, hd=500):
	wd = min(wd,ui.get_screen_size()[0])
...
class Outliner(ui.View):
	...
	def textview_did_begin_editing(self, textview):
		...
		if yp > self.keyboard_y:
			...
			y = self.tv.content_offset[1] + self.keyboard_y - 100
