...
todo
====
- bug: set_outline_button border=1 shows big buttons for small hidden outlines
- move/drag/drop
	.. when moving, if pass on outline, show it (other color?) or bold
	.. drop: process
	..				- delete old lines
	..				- loop add lines one by one
- .. long press move left or right simulate tab / back tab
- ok begin automatically with an outline
- ok memo last open file and open it at start
- ok tab
...
-	.. lf renumbering
-	ok back tab
-	ok renumering back tab
-	.. cursor after
- .. quid if paste more than one, do not allow?

niceties optional todo
======================
- .. move,copy, drag/drop....renumbering, quid not same level? from and to location
...
Version V00.05
  ...
  - to start a drag operation, long press anywhere on a line
    ...
    - drop the mobile label so its top/left blue point falls on a line
...
class Outliner(ui.View):
	def __init__(self, *args, **kwargs):
		...
		self.text_attributes_hidden = {NSForegroundColorAttributeName:text_color, NSFontAttributeName:font_hidden}
	...
	def frame_of_part(self,tv,i,n):
		tvo = ObjCInstance(tv)
		p1 = tvo.positionFromPosition_offset_(tvo.beginningOfDocument(), i)
		p2 = tvo.positionFromPosition_offset_(tvo.beginningOfDocument(), i+n)
	...
	def set_outline_button(self,i,n):
		x,y,w,h = self.frame_of_part(self.tv,i,n)
		#print(x,y,w,h)
		b = ui.Button()
		b.outline = i
		b.frame = (x,y,w,h)
	...
	def long_press_handler(self,data):
		...
		if data.state == 1:
			...
			# move
			if self.tvm:
				self.tvm.x, self.tvm.y = (xp,yp-100)
		elif data.state == 3:
			# end
			if self.tvm:
	...
	def xy_to_i(self,xp,yp):	
		...
		for sv in self.tv.subviews:
			if isinstance(sv,ui.Button):
				caret_pos = self.tvo.selectedTextRange().start()
				caret_pos.setOffset_(sv.outline)
				rect = self.tvo.caretRectForPosition_(caret_pos)
				(x,y) = rect.origin.x, rect.origin.y
				(w,h) = rect.size.width, rect.size.height
				#print(yp,y,y+h)
				if y <= yp and yp <= (y+h):
