...
- bug: 0️⃣ I typed a line and when I got to the end on the right => error
					*** not reproductible ***
- bug: 0️⃣ https://i.imgur.com/TcWTDs
- bug: 0️⃣ the date column label and sort button seem to disappear at times 
					(perhaps after moving an item? 
- bug: 0️⃣ I also managed to lose the top row of icons when I used collapse all
					to hide the date column..
- bug: 0️⃣ undo: bad successive 
					- multiple undo's' is heavy about meory usage
					- setting? multiple undo yes/no or number of successive?
- bug: 🔟 add/del characters which change attribs looses cursor after redisp row
- new: 🔟 color of line of strikethrough and underline (wait answer of @ihf or do!)
					- color of outline border (stroke)?, filled?
					- background color?
					- oblique?
					- double strike
- new: 2️⃣ 2nd arg as a path to open 1st arg outline from other path than current
					should work for arg1 = file 
											for arg1 = .ext 
- new: 🔟 create drawing as image, save where?
'''
import appex
import ast
import collections
...
class Outliner(ui.View):
				...
				while 1 == 1:
					with ui.ImageContext(w,h) as ctx:
						...
						while row < len(self.tv.data_source.items):
							...
							if attribs:
								# opts['attribs'] = [(s,e,'biusohc'),(s,e,'B'),...]
								for attrib in attribs:
									sa,ea,biusohc = attrib
									attributes = self.get_attributes(biusohc)
	...
	def get_attributes(self, biusohc, font_size=None):
		if not font_size:
		...
		if 'o' in biusohc:
			attributes[NSStrokeWidthAttributeName] = 3.0 # >0: stroke only <0: stroke+fill
			#attributes[NSStrokeColorAttributeName] = ObjCClass('UIColor').redColor()
		if 'c' in biusohc:
			# c(r,g,b)
			i = biusohc.find('(')
			j = biusohc.find(')',i)
	...
	def textview_should_change(self, textview, range_c, replacement, play=False, key=False):
		...
		if replacement == '\x01':
			...
			if attribs_old:
				# transform all attribs of line into array of '' or biusohc
				arr = [''] * len(t)
				for attrib in attribs_old:
					s,e,biusohc = attrib
					arr = arr[:s] + [biusohc]*(e-s) + arr[e:]
				...
				for biusohc in arr:
					if biusohc:
						if prev_biusohc:
							if biusohc != prev_biusohc:
								# terminates previous attrib
								e = i
								attrib = (s,e,prev_biusohc)
			...
			if ho_after != ho_before:
				...
				if attribs != attribs_old:
					#self.cells[row].content_view['text'].text = t
					#self.redraw_textview_attributes(textview,opts)
					ui.delay(partial(self.tableview_reload_one_row, row), 0.1)
	...
	def selected_text_attributes(self,row):
		...
		def b_action(sender):
			...
			for attrib_old in attribs:
				s_old,e_old,biusohc_old = attrib_old
		...
		lc = ui.Label()
		lc.frame = (10,238,110,32)
		lc.text = 'color'
		v.add_subview(lc)
		sc = ui.Button(name='c')
		sc.frame = (144,238,30,30)
		sc.border_width = 1
		...
		def color_action(sender):
			...
			if 'c' in v.biusohc:
				...
				i = v.biusohc.find('(')
				j = v.biusohc.find(')',i)
				v.biusohc = v.biusohc[:i-1] + v.biusohc[j+1:]
			v.biusohc += 'c('+str(r)+','+str(g)+','+str(b)+')'
			#print(v.biusohc)
			attrText = NSMutableAttributedString.alloc().initWithString_(l.text)
			attributes = self.get_attributes(v.biusohc, font_size=24)
		...
		def sw_action(sender):
			...
			v = sender.superview
			a = sender.name # b or i or u or s or h
			v.biusohc = v.biusohc.replace(a,'')
			if sender.value:
				v.biusohc += a
			l = v['text']
			attrText = NSMutableAttributedString.alloc().initWithString_(l.text)
		...
		if attribs:
			...
			for attrib in attribs:
				...
				if sa <= s and e <= ea:
					...
					v.biusohc = biusohc
					sb.value = 'b' in biusohc
					si.value = 'i' in biusohc
					su.value = 'u' in biusohc
					ss.value = 's' in biusohc
					so.value = 'o' in biusohc
					if 'c' in biusohc:
						i = biusohc.find('(')
						j = biusohc.find(')',i)
						rgb = biusohc[i+1:j]
						...
						sc.tint_color = (r,g,b)
					attrText = NSMutableAttributedString.alloc().initWithString_(l.text)
					attributes = self.get_attributes(biusohc, font_size=24)
					attrText.setAttributes_range_(attributes, NSRange(0,len(l.text)))
					self.set_attributed_text(ObjCInstance(l), attrText)	
					break				
