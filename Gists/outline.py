...
==== 
- new: 1️⃣ Wish list: mail/pushover/notification on due date/time-
						- remove if 
							- file save refuse
							  ***** if save, create events just before)
							- row removed (ihf says not needed)
							- due date unset (ihf says not needed)
							- due date changed (ihf says not needed)
							- due date reached since some time (ihf says not needed)
								- each run, scan events of calendar,  of date < today-1day and delete them
...
- new: 🔟 - 2nd color for underline, strike, outline fill, shadow
						*** attention: support o alone and o(r,g,b) in existing opts['attribs']
					- exponent baselineOffset = font_size/4 font_size=1/2
					- indice font_size=1/2
					- image in switch a²
					- switchs images do not use color for character a
...
class Outliner(ui.View):
	def __init__(self, *args, **kwargs):
		...
		self._handlers = [
				register_key_event_handler(UIEventKeyCode.RIGHT, self.external_right, modifier=UIKeyModifier.COMMAND),
				register_key_event_handler(UIEventKeyCode.LEFT, self.external_left, modifier=UIKeyModifier.COMMAND)
				]
	...
	def set_due_date(self, view, unset=False):
		row = view.row
		vals,n,opts = self.tv.data_source.items[row]['content']
		dc,du,dd,de = opts['dates']
		if unset:
		...
		if dd:
			if dt > datetime.now():
				...
				if self.cal:
					...
					self.store.saveEvent_span_error_(event, span, None)
