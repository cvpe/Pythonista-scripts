...
todo 
====
- .. Q3: replace the _ of outlines by a space?
...
class Outliner(ui.View):
	def __init__(self, *args, **kwargs):
		...
		self.right_button_items = (b_format, b_color, b_font, b_fsize, b_search, b_undo, b_show)
		...
		if 'delete in red' not in self.prms:
			self.prms['delete in red'] = 'yes'
		self.red_delete = self.prms['delete in red']
	...
	def button_undo_action(self,sender):
		if sender.title.startswith('un'):
			self.redo_items = self.tv.data_source.items.copy()
			self.tv.data_source.items = self.undo_items.copy()		
			self.button_undo_enable(True,'')
		else:
			self.tv.data_source.items = self.redo_items.copy()			
			self.button_undo_enable(True,sender.title[2:])
		self.tv.reload_data()
		
	def undo_save(self,action):
		self.undo_items = self.tv.data_source.items.copy()
