...
- .. new: multiple level undo
					...
					- quid if action = text or left delete
					- clear array if new/open file
					- not save all but modifs only? not easy
...
class Outliner(ui.View):
	...
	def layout(self):
		...
		self['title'].frame = (0,10,self.width,30)
		x = dd
	...
	def undo_save(self,action):
		...
		#print(sys.getsizeof(self.undo_items))
	...
	def tableview_cell_for_row(self, tableview, section, row):
		...
		if not hidden:	
			...
			if self.show_lines_separator == 'yes':					
				sep = ui.Label()
				sep.frame = (0,ho-1,self.tv.width,1)
				sep.border_width = 1
	...
	def single_or_double_tap_handler(self,data):
		...
		v = data.view
		row = v.row
		#ui.delay(self.cells[row].content_view['text'].end_editing,0.0)
		#v.border_width = 1
