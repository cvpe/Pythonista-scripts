...
todo 
====
- .. bug: no renumbering after drop
	
- .. Q?: outline as buttons: long press and (double) tap (script: search 19)

- .. option renum all de rattrappage...
...
class Outliner(ui.View):
			...
			with open(path+log, mode='rt') as fil:
				...
				for rec in recs[:-1]:
					...
					if fs[0] == 'drop':
						...
						self.drop(found, fm, tm, play=True)
					else:
	...
	def textview_should_change(self, textview, range_c, replacement, play=False):
		...
		if replacement == '\x01':
			...
			if len(nvals) >= 1 and not same_outline:			
				...
				fr = range_c[0]+1
				self.renumbering('lf', fr, vals_before, nvals)		
		...
		if data.state == 1:
			...
			if self.tvm:
				...
				if found == line1:
					...
					self.drop(self.found_redline,fm,tm)
					del self.drag
	...
	def drop(self,found, fm, tm, play=False):
		#print('drop:', found, fm,tm, len(self.tv.text))

		# eventual log
		if self.log == 'yes' and not play:
			rec = f"drop,{found},{fm},{tm}"
			rec += lf
			self.log_fil.write(rec)
		...
		# renumbering old after all inserted lines
		self.renumbering('lf',found+len(replacement)-1, None, vals1)	
		# remove original
		if tm > found:
			# deleted part after insertion, thus increase indexes
			fm += len(replacement)	# needed for removal
			tm += len(replacement)
		# if copy instead of move, comment these lones: begin ========
		self.tv.text = self.tv.text[:fm] + self.tv.text[tm:]
		self.content = self.content[:fm] + self.content[tm:]
		
		# renumbering after old removed
		self.renumbering('del',fm-1, vals1, None)		
		# if copy instead of move, comment these lones: end  ==========
	...
	def set_outline_attributes(self):			
		...
		for sv in self.tv.subviews:
			if isinstance(sv, ui.Label):
				self.tv.remove_subview(sv)
				del sv
		# V00.19: begin ======================
		'''		
		# build buttons at left of outline for hide/show children: begin =======
		...
		while i < len(self.tv.text):
			if isinstance(self.content[i], tuple):
				...
				if build_b:
					...
					bb.border_width = 1
					def bb_tap(data):
						data.location.x += bb.x
						data.location.y += bb.y - 70
						self.doubletap_handler(data)
					def bb_longpress(data):
						self.long_press_handler(data)
					tap(bb,bb_tap)
					long_press(bb,bb_longpress)
			...
			if ilf < 0:
				ilf = len(self.tv.text)
			i = ilf + 1		
		# build buttons at left of outline for hide/show children: end =========
