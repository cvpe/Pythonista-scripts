...
todo 
====
- new: 1️⃣ generate rtf? so font used?
- new: 1️⃣ support level 0 = [] > no outline wait answer @ihf
					- ❗️doubletap on checkbox to get popup menu				
					- format types check title yes/no 
...
- new: 🔟 self.auto_save seems to not be functional
...
class Outliner(ui.View):
	...
	def tableview_cell_for_row(self, tableview, section, rowin):
		...
		if row == self.cursor[0] and len(self.undo_multiples) > 0:
			if self.undo_multiples[-1][0] == 'CR':# or (row == 0 and txt == ''):
				bb.begin_editing()
