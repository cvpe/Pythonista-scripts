...
- new: 1️⃣ calendar events for due date
					- 2️⃣update txt/outline => update event title, notes, if event exists
					- 🔟rename file => update event title, notes, url
	...
	def set_due_date(self, view, unset=False, delete=None):
		if delete:
			...
			if event:
				...
				ret = console.hud_alert('due date event has been removed','success',3)
				if ret:
...
def main():
	...
	if len(sys.argv) == 2:
		if sys.argv[1] == 'grant_access_to_calendar':
			...
			print('see IOS Settings/Pythinista')
