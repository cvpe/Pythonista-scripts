...
- new: 🔟 fonts... per level (very heavy development)
					- show grid		 	0  1 2 3 4 5 6 <-- levels
								font			f1
								font size			10
								color
								bold				V
	...
	def files_action(self, sender, act):
		...
		if act == 'New':
			...
			if not self.file:
				...
				return
			if 'Christian' in self.device_name:
				b = console.alert('Share as','', '.txt','.rtf','cancel', hide_cancel_button=True)
			else:
				b = 1
			if b == 3:
				return
					...
					for l in ls:
						if first:
							if path[-3:] == 'txt':
								...
								lvl = len(item['content'][0])	# level of vals
								to = outline
								if lvl == 1:
									t += "{\\pard {\\f0\\cf1\\fs32\\b "+to+"}{\\f0\\cf2\\fs32\\b "+l+"}\\par}"
								else:
									t += "{\\pard {\\f0\\cf1\\fs24 "+to+"}{\\f0\\cf2\\fs24 "+l+"}\\par}"
							first = False
						else: 
							if path[-3:] == 'txt':
