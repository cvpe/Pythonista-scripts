...
- new: 🔟 calendar events for due date, when seems ok for @ihf
					- delete with children: delete events
					- delete selected: delete events
					- cut selected: delete events
					- paste: add events
...
- new: 2️⃣ 2nd arg as a path to open 1st arg outline from other path than current
					should work for arg1 = file 
											for arg1 = .ext 
- new: 0️⃣ font_size by level
					- quid for outline "1.2" nicer if same size for each level?
					- bouton level size
...
class Outliner(ui.View):
	...
	def tableview_cell_for_row(self, tableview, section, rowin):
		...
		if not hidden:
			if self.select:
				...
				#		due_date = dates[2]
				#		if due_date:
				#			chk.background_color = (0,0,1,0.1)
	...
	def set_due_date(self, view, unset=False, delete=None):
		if delete:
		...
		if dd:
			if self.cal:
				...
				event.addAlarm_(a)
				event.title = f"{self.file} : {self.tv.data_source.items[row]['outline']} {self.tv.data_source.items[row]['title'][:20]}"
				event.notes = f"pythonista3://outline.py?action=run&args={self.file}\n\n{self.tv.data_source.items[row]['title']}"
