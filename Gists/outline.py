...
todo 
====
- bug: 0️⃣ incoherence between text and content files 
					https://i.imgur.com/2XohBym
- new: 0️⃣ check if better way to check if .content downloaded
- new: 2️⃣ 2nd arg as a path to open outline from other path than current
					should work for arg1 = file or ext => to be tested
- bug: 1️⃣ tab => bad successive 
...
- new: 🔟 print text+images in program btn=printer or submenu files, option print
				
- new: 🔟 support level 0 = [] > no outline
					- doubletap on checkbox to get popup menu
					
- new: 🔟 main menu: style => modif font,font_size, color for each level
					- memo not per line but per file in .content general, like format type
- new: 🔟 functionnality to build/modify user format?
...
class Outliner(ui.View):
			...
			if parm.startswith('.'):
				...
				if not os.path.exists(path+file):
					...
					if not os.path.exists(path+file_content):
						console.alert('file argument has no .content','error', 3)
					else:
						self.prms['path'] = path
						self.prms['file'] = file										
	...
	def files_action(self, sender, act):
		...
		if act == 'New':
			...
			if not self.file:				
				...
				ext = f[j:].lower()
				if ext == '.content':
					console.hud_alert('name .content not allowed','error',3)
					return
				self.file = f
			...
			if not os.path.exists(self.path+self.file):
				console.hud_alert('old file does not exist','error', 3)
				return
			if not os.path.exists(self.path+self.file_content):
				console.hud_alert('. content does not exist','error', 3)
				return
			# ask new name
			f = console.input_alert('Name of new file', hide_cancel_button=True)
			if not f:
			...
			if os.path.exists(self.path+f):
				...
				return
			if os.path.exists(self.path+file_content):
				console.hud_alert('.content already exists','error', 3)
				return
			# rename
	...
	def pick_open_callback(self,param):
		...
		if param:
			...
			ext = f[j:].lower()
			if ext == '.content':
				console.hud_alert('Open .content file not allowed','error', 3)
				return
			self.path = f[:i+1]
			self.file = f[i+1:]
		else:
		...
		self.file_content = self.file + '.content'
		# force download content
		# force download file
		if '/private/var/mobile/Library/Mobile Documents/' in self.path:
			# icloud file
			# force download
			url = nsurl('file://' + (self.path+self.file_content).replace(' ','%20'))
			NSFileManager = ObjCClass('NSFileManager').defaultManager()
			ret = NSFileManager.startDownloadingUbiquitousItemAtURL_error_(url, None)
			#print(url,'downloaded',ret)	# True if ok
		if not os.path.exists(self.path+self.file_content):
			...
			if '/private/var/mobile/Library/Mobile Documents/' in self.path:
				console.hud_alert('.content does not (yet) exist, retry', 'error', 3)
			else:
				console.hud_alert('.content does not exist', 'error', 3)
			return
		...
		if i < 0:
			i = len(self.file)
		with open(self.path+self.file,mode='rt') as filt:
			t = filt.read()
		lines = t.split(lf)
		del t
		with open(self.path+self.file_content,mode='rt') as filc:
			c = filc.read()
		c_prms = c.split(lf)
		c = c_prms[0]
		prms = c_prms[1]
	...
	def tableview_cell_for_row(self, tableview, section, rowin):
		...
		if (row == self.cursor[0] and self.undo_multiples[-1][0] == 'CR'):# or (row == 0 and txt == ''):
			bb.begin_editing()
	...
	def size_segment_tap(self,sender):
		...
		n = sender.dims[sender.segments[idx]]
		v = sender.superview
	...
	def file_save(self, save_prm=True):
		...
		# as protection if any problem during save
