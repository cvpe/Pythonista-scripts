...
- new: 1️⃣ support bold/italic/strikethrough/underline font
					- not yet supported: delete or insert in a attributed portion
						❗️temporary remove attribs if overlap insert or delete part❗️
...
NSStrokeColor = ns('NSStrokeColor')
NSStrokeWidth = ns('NSStrokeWidth')
...
class Outliner(ui.View):
				...
				while 1 == 1:
					with ui.ImageContext(w,h) as ctx:
						...
						while row < len(self.tv.data_source.items):
							...
							if attribs:
								# opts['attribs'] = [(s,e,'biuso'),(s,e,'B'),...]
								for attrib in attribs:
									sa,ea,biuso = attrib
									attributes = self.get_attributes(biuso)
	...
	def get_attributes(self, biuso):
		traits = 0
		if 'b' in biuso:
			traits = traits | 1 << 1
		if 'i' in biuso:
			traits = traits | 1 << 0
		font = UIFont.fontWithName_size_traits_(self.font, self.font_size,traits)
		attributes = {NSForegroundColorAttributeName:self.text_color, NSFontAttributeName:font}								
		if 'u' in biuso:
	...
	def textview_should_change(self, textview, range_c, replacement, play=False, key=False):
		...
		if replacement == '\x01':
			...
			# in case of insert or delete characters, we have ro check
			# if we need to change some attributes ranges
			attribs_old = opts.get('attribs',None)
			if attribs_old:
				attribs = []
				for old_attrib in attribs_old:
					s,e,biuso = old_attrib
					# check if range [s,e-1] has to be changed
					if e <= range_c[0]:
						# attrib before any insert or delete: range unmodified
						pass
					elif s >= range_c[1]:
						# attrib after any insert or delete: swipe range
						s += l_aft - l_bef
						e += l_aft - l_bef
					else:
						# attrib overlaps insert or delete part: remove it
						continue
					attrib = (s,e,biuso)
	...
	def selected_text_attributes(self,row):
		...
		def b_action(sender):
			...
			for attrib_old in attribs:
				s_old,e_old,biuso_old = attrib_old
				if s <= s_old and e_old <= e:
					...
					if s_old < s:
						attrib_old1 = (s_old,s,biuso_old)
						attribs_new.append(attrib_old1)
					if e < e_old:
						attrib_old2 = (e,e_old,biuso_old)
						attribs_new.append(attrib_old2)
				elif s < s_old and e < e_old:
					# new overlaps left part of old
					attrib_old1 = (e,e_old,biuso_old)
					attribs_new.append(attrib_old1)					
				elif s > s_old and e > e_old:
			...
			for sv in ObjCInstance(sender).subviews():
				if sv._get_objc_classname().startswith(b'UISwitch'):
					with ui.ImageContext(32,32) as ctx:
						...
						attributes = self.get_attributes(t)
		...
		def sw_action(sender):
			...
			v = sender.superview
			a = sender.name # b or i or u or s
			v.biuso = v.biuso.replace(a,'')
			if sender.value:
				v.biuso += a
			l = v['text']
			attrText = NSMutableAttributedString.alloc().initWithString_(l.text)
			attributes = self.get_attributes(v.biuso)
