...
todo
====
-	.. bug: imbed tab level > previous tab of more than 1, search previous,skip hidden
- .. move/drag/drop
	.. 	- drop: process
	..		- paste before of after target line?
	..		- delete old lines
	..		- loop add lines one by one, use self.drag_content
-	.. lf insert
-	.. lf renumbering
- .. quid if paste more than one, do not allow?
- .. several lines with same outline, how to set it (no lf?)
- .. 🔟content: ([1,2,3],nchr,{'hidden':True, 'style':(font,fontsize,bold,other)}
- .. 🔟popup: style => modif font,font_size for one line
- .. 🔟search text, show only items containing search and their parants
...
- ..	- tap detail icon => tableview des differents levels
...
class Outliner(ui.View):
	...
	def popup_menu_action(self,sender):
	...
	def drag_children(self,fr,xp,yp):
		...
		while k >= 0:
			if k >= len(t):
				...
				break
		self.drag = t[fr:k1]
		self.drag_content = self.content[fr:k1]
		#print(self.drag)				
		if not self.drag:
	...
	def OutlineFromLevelValue(self,vals):
		...
		if self.outline_format == 'decimal':
			...
			blanks = 2
		elif self.outline_format in ['alphanumeric','traditional','bullets']:
			if self.outline_format == 'alphanumeric':
				...
				blanks = 3
			elif self.outline_format == 'bullets':			
				outline_types = ['•', '‣', '◦', '⦿', '⁃', '⦾', '◘.']
				blanks = 2
