...
- bug: 🔟 add/del characters which changes attribs looses cursor after redisp row
...
class Outliner(ui.View):
	...
	def textview_should_change(self, textview, range_c, replacement, play=False, key=False):
		...
		if replacement == '\x01':
			...
			if attribs_old:
				# transform all attribs of line into array of '' or biuso
				arr = [''] * len(t)
				for attrib in attribs_old:
					s,e,biuso = attrib
					arr = arr[:s] + [biuso]*(e-s) + arr[e:]
