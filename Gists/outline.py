...
todo
====
- new: 1️⃣ Wish list: send reminders on due date/time via email or
					perhaps pushover notification(easy to do in Python).
- bug: 0️⃣ image left, begin to type text, too high
...
class Outliner(ui.View):
	...
	def get_attributes(self, biusohcqg, font_size=None):
		...
		if 'o' in biusohcqg:
			...
			# if strokewidth < 0: foreground color = fill    strokecolor = 2nd color
			if 1==1:
				...
				attributes[NSForegroundColorAttributeName] = UIColor.colorWithRed_green_blue_alpha_(0,1,0, 1)
		if 'h' in biusohcqg:
	...
	def set_due_date(self, datepicker):
		...
		# set a system notification to warn the user at the due date/time		
		attachments = None
		img = opts.get('image', None)
		if img:
			attachments = [img[0]]
		# url to start Pythonista running this program
		pgm = sys.argv[0]
		t = 'Pythonista3/Documents/'
		i = pgm.find(t)
		pgm = pgm[i+len(t):]		
		url = 'pythonista3://' + pgm + '?action=run'
		#print(url)
		delay_sec = (dt - datetime.now()).seconds
		notification.schedule(message=self.tv.data_source.items[row]['title'], delay=delay_sec, title='Outline notification: due date reached for ' + self.tv.data_source.items[row]['outline'], subtitle=None, attachments=attachments, sound_name='default', action_url=url)
	...
	def selected_text_attributes(self,row):
		...
		def color_action(sender):
			...
			v.biusohcqg += sender.name + '('+str(r)+','+str(g)+','+str(b)+')'
			#print(v.biusohcqg)
