...
todo 
==== 
- ask: 0️⃣ questions, little improvements:
					- external keyboard: special keys combinations for
						- font attributes
						- some frequently used options of the popup menu
						- dynamic?
						functionalities: combination
						array ex: promote Ctrl/Left modifier key both in existing table
...
- new: 1️⃣ calendar events for due date
					...
					- 🔟 ask: rename file => update event title, notes, url
						- scan all items, if due date eventid, update	title and url			r
- bug: 0️⃣ I typed a line and when I got to the end on the right => error
					*** not reproductible ***
- new: 🔟	If the program gets a wider audience you might also consider making
					...
					- calendar events yes/no option 
						- import blackmamba only of option is yes
...
class Outliner(ui.View):
	def __init__(self, *args, **kwargs):
		...
		if 'external keyboard' not in self.prms:
			self.prms['external keyboard'] = 'no'
		self.external_keyboard = self.prms['external keyboard']
	...
	def set_key_event_handlers(self):
		...
		# register new
		self._handlers = []
		if self.external_keyboard != 'no':
			modifier = {'Alt':UIKeyModifier.ALTERNATE, 'Ctrl':UIKeyModifier.CONTROL, 'Cmd':UIKeyModifier.COMMAND, 'Shift':UIKeyModifier.SHIFT}[self.external_keyboard]
			self._handlers = [
				register_key_event_handler(UIEventKeyCode.RIGHT, self.external_right, modifier=modifier),
				register_key_event_handler(UIEventKeyCode.LEFT, self.external_left, modifier=modifier)
				]
		
	def external_right(self):
	...
	def button_settings_action(self,sender):
		...
		fields.append({'title':'external keyboard modifier', 'type':'text', 'value':self.external_keyboard, 'key':'segmented13', 'segments':['no', 'Alt', 'Cmd', 'Ctrl', 'Shift']})
		...
		old = self.external_keyboard		
		self.external_keyboard = f['external keyboard modifier']
		self.prms['external keyboard'] = self.external_keyboard
		if self.external_keyboard != old:
			self.set_key_event_handlers()
	...
	def tableview_did_select(self, tableview, section, row):
		...
		if tableview.name == 'select':					
			# all devices: no present modal, no shiel
	...
	def paste(self, found):
		...
		while row <= (len(self.paste_items)-1):
			...
			if dd:
				# due date exists
				title = item['title']
				self.set_due_date((dd,title), feedback=False)
	...
	def set_due_date(self, view, unset=False, delete=None, feedback=True):
		if delete:
			...
			if event:
				...
				ret = self.store.removeEvent_span_error_(event,0,None)
				if not feedback:
					return
				if ret:
		...
		if isinstance(view, tuple):
			# view is already (dd,title), passed when  paste adds a due date event
			dd, title = view
			dt = datetime.strptime(dd,'%Y-%m-%d %H:%M:%S')
			dd_old = None # just to force that no 'eventid' known
		else:
			# view is an ui.View
			row = view.row
			vals,n,opts = self.tv.data_source.items[row]['content']
		...
		if dd:
			if self.cal:
				...
				if isinstance(view,tuple):
					return # id will be store in calling code
				opts['eventid'] = id
			else:
	...
	def single_or_double_tap_handler(self,data):
		...
		sub_menu = ['hide children', 'show children', 'delete with children', '⏭', '⏮', '🖼','bold ...']
		...
		ok = True
		try:
			with open('outline.clipboard', mode='rt') as fil:
				clipboard_str = fil.read()			
			#clipboard_str = clipboard.get()
			#print(clipboard_str)
			self.paste_items = ast.literal_eval(clipboard_str)
			del clipboard_str
	...
	def selected_text_attributes(self,row):
		...
		mv.present_popover(v, 'popover',popover_location=(x,y),hide_title_bar=False, force=True)	
		v.height = min(v.content_size[1],self.height - 40)
		v.y = (self.height - v.height) / 2
...
def main():
	...
	# normal mode
	if len(sys.argv) == 2:
		if sys.argv[1] == 'grant_access_to_calendar':
