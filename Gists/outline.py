...
- new: 2️⃣ I was thinking that if one wanted to add a line (an item) to an outline,
					...
					- arg1 = file name
					- appex.get text
					- read .prm => current path
					- get latest file beginning with arg1
					- read file
					- add outline
					- save
...
Versions = '''
Version V00.91
  - support run in appex mode as share of text to add it at end of an outline
...
class Outliner(ui.View):
	...
	def search_files(self, search_text):
		txt = unicodedata.normalize('NFKD', search_text).encode('ASCII', 'ignore')
		txt = str(txt,'utf-8').upper()
		files = os.listdir(path=self.current_path)
		sub_menu = []
		for f in files:
			if f.endswith('.outline'):
				with open(self.current_path+f, mode='rt') as fil:
					c = fil.read()
					c_prms = c.split(lf)
					c = c_prms[0]
	...
	def selected_text_attributes(self,row):
		#print('selected_text_attributes')
...
def main():
	...
	if appex.is_running_extension():
		if len(sys.argv) > 1:
			...
			txt = appex.get_text()
			if not txt:
				console.hud_alert('No text passed', 'error',3)
				appex.finish()
				return	
			...
			if not os.path.exists(path+file):
				...
				return	
			# read file
			with open(path+file,mode='rt') as filc:
				c = filc.read()
			c_prms = c.split(lf)
			c = c_prms[0]
			prms = c_prms[1]
			items = ast.literal_eval(c)
			prms = ast.literal_eval(prms)
			n = 0
			for item in items:
			...
			vals = [n]
			mv = Outliner()	
			mv.first_level_has_outline = prms['1st level has outline']		
			mv.outline_format = prms['format']
			...
			with open(path + new_filename, mode='wt') as fil:
				t = str(items) + lf + str(prms)
				fil.write(t)	
			os.remove(path + file)		
			console.hud_alert('text pasted in '+new_filename,'success', 3)
			appex.finish()
			return
		#=======================================				
