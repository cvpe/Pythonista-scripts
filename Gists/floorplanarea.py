#!python3
# basd on original version of @jonB https://github.com/jsbain/RoomAreaFinder
# June 16 2025: modified by cvp to use TextField value as real length of 
#               distance between the two first points
# coding: utf-8
...
class RoomAreaView(ui.View):
	...
	def __init__(self,file):
		...
		elif isinstance(file, ui.Image):
			iv.image = file
		else:
			print("invalid parameter file: ", file)
			return
	...
	def will_close(self):
		...
		if self.pts:
			x,y=zip(*self.rv.pts)
			print (polygonArea(x,y,self.rv.scale))
class RoomAreaOverlay(ui.View):
	...
	def __init__(self,*args,**kwargs):
		...
		self.curridx=-1
		self.scale = 1
		# create a textfield to set distance brtween two first points. 
		# could be a slider instead
		...
		self.l12=ui.TextField(frame=(50,0,200,30))
		self.l12.placeholder = "real distance points 1-2"
		self.l12.action=self.set_scale
		self.add_subview(self.l12)
		self.l12.text=''
		# create a label showing current computer room area
		self.lbl=ui.Label(frame=(300,0,130,30))
		self.lbl.text='Area'
	...
	def set_scale(self,sender):
		...
		if self.pts:
			if len(self.pts) >= 2:
				L=(ui.Point(*self.pts[1])-ui.Point(*self.pts[0]))
				self.scale = abs(L)/float(self.l12.text)
		self.update_area()
	...
	def update_area(self):
		...
		try:
			x,y=zip(*self.pts)
			area=polygonArea(x,y,self.scale)
	...
	def draw(self):
		...
		if drawpts:
			...
			for p in drawpts[1:]:
				...
				ui.draw_string('%3.2f'%abs(L/self.scale), (H.x,H.y,0,0),color='red')
...
asset = photos.pick_asset(assets=images, multi=False)
if asset:
	file = asset.get_ui_image()
else:
	file = '203723572.jpg'
v=ui.View()
rv=RoomAreaView(file)
rv.flex='wh'
v.add_subview(rv)
#v.present('panel')
