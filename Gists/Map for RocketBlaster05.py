		...
		if annotation.isKindOfClass_(MKPointAnnotation):
			...
			if not pinView:
				if tit != 'Current Location':
					...
					pinView.setImage_(my_ui_image.objc_instance)			
...
def main():
	...
	global my_ui_image											# for photo as pin
	my_ui_image = ui.Image.named('emj:Man')	# for photo as pin
	#my_ui_image = ui.Image.named('test:Mandrill')	# for photo as pin
	d = 28
	...
	with ui.ImageContext(28,43) as ctx:	
		foot = ui.Path.oval(12,41,4,2)
		ui.set_color((0,0,0,1))
		foot.fill()
		pin = ui.Path.rounded_rect(13,17,2,25,1)
		ui.set_color((0.6,0.6,0.6,1))
		pin.fill()
		my_ui_image.draw(0,0,d,d)
		#blood = ui.Path.oval(14,26,4,8)
