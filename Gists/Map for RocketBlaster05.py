# todo
# - overlap of several circles of all points of a deleted group
#		- is it possible to only get the union of the circles
#		  see https://stackoverflow.com/questions/29204125/mkcirclerenderer-fillcolour-overlap-issue 
#		- MKOverlayPathRenderer -> CGPath = path.CGPath 
#			where path = UIBezierPath() and objinstance(ui.Path) is an UIBezierPath
'''
	...
	if title == 'Current Location':
		...
		if title == 'Current Location':
			if (lat,lon) in locs:
				...
				return
	if title in ['deleted user point', 'deleted group']:
		latu,lonu = mapView.ui_view.user
		d = haversine(lat,lon,latu,lonu)	
		if d > (2*close_distance):
			console.alert('Your location is too far', 'delete pin is refused', 'ok', hide_cancel_button=True)
			return
	if title in ['deleted user point']:
		mapView.ui_view.remove_one_pin(annotation)
		del locs[(lat,lon)]										# remove trashed pin 
		update_file()													# update file
		return
	elif title in ['deleted group']:
		nbr_needed = locs[(lat,lon)][-1]
		nbr_needed -= 1
		if nbr_needed > 0:
			locs[(lat,lon)][-1] = nbr_needed
			update_file()													# update file
			mapView.ui_view.remove_one_pin(annotation)
			# recreate it to refresh content
			annotg = mapView.ui_view.add_pin(lat, lon, 'deleted group', subtit, select=True)		
			return
		# nunber needed = 0 => remove pin and its circles
		#......circles of all points
		mapView.ui_view.remove_one_pin(annotation)
		del locs[(lat,lon)]										# remove trashed pin 
		update_file()													# update file
		return
	...
	for annotation1 in mapView.annotations():
		if str(annotation1.title()) == 'deleted group':
			...
			if all_close:
				...
				mapView.ui_view.addCircleToMap(lat,lon,close_distance,str((lat,lon)))
				...
				# remove trashed pin
				mapView.ui_view.remove_one_pin(annot_new)
				# update locs and  file
				del locs[(lat,lon)]							# remove trashed pin 
				nbr_needed = locs[(lat1,lon1)][-1]
				del locs[(lat1,lon1)]						# remove old group
				locs[(latc,lonc)]	= close_pins	# add new group
				nbr_needed += 1
				locs[(latc,lonc)].append(nbr_needed)				
		...
		if str(annotation1.title()) == 'deleted user point':
			...
			if len(close_pins) >= close_number:
				...
				for annot in close_annots:
					...
					mapView.ui_view.remove_one_pin(annot)
				...
				for lat,lon in close_pins:
					...
					mapView.ui_view.addCircleToMap(lat,lon,close_distance,str((lat,lon)))
					mapView.ui_view.addCircleToMap(lat,lon,0.3,str((lat,lon)))			
				annotg = mapView.ui_view.add_pin(latc, lonc, 'deleted group', subtit_new)	
				#circle = mapView.ui_view.addCircleToMap(latc,lonc,close_distance,subtit_new)
				# store circle overlay for annotation because we could remove it
				#annotg.circle = circle				
				locs[(latc,lonc)]	= close_pins
				nbr_needed = len(close_pins)
				locs[(latc,lonc)].append(nbr_needed)				
...
def mapView_viewForAnnotation_(self,cmd,mk_mapview,mk_annotation):
	global own_ui_image, del_ui_image, grp_ui_image, locs
	try:
		...
		if annotation.isKindOfClass_(MKPointAnnotation):
			...
			subtit = str(annotation.subtitle())
			if tit == 'deleted group':
				nbr_needed = locs[(lat,lon)][-1]
			else:
				nbr_needed = 0 # unused
			id = tit + subtit + str(nbr_needed)
			...
			if not pinView:
				...
				elif tit in ['deleted user point', 'deleted group']:
					# RightCalloutAccessoryView for trash button		
					bo = ObjCClass('UIButton').alloc().init()
					bo.setTitle_forState_('🚮',0)
					bo.setFrame_(CGRect(CGPoint(0,0), CGSize(40,40)))				
					pinView.setRightCalloutAccessoryView_(bo)		
					if tit in ['deleted group']:					
						h = 39
						d = 10
						l_nbr = ui.Label()
						l_nbr.frame = (0,h+4,h,d)
						l_nbr.font = ('Menlo',d)
						l_nbr.text_color = 'red'
						l_nbr.text = 'needed'
						l_nbr.alignment = ui.ALIGN_CENTER
						l_nbr.background_color = (1,1,1,0.8)	# délavé
						ol_nbr = ObjCInstance(l_nbr)
						bo.addSubview_(ol_nbr)	

						d = 12
						l_nbr = ui.Label()
						l_nbr.frame = (0,h-d+4,h,d)
						l_nbr.font = ('Menlo',d)
						l_nbr.text_color = 'red'
						l_nbr.text = f"({nbr_needed})"
						l_nbr.alignment = ui.ALIGN_CENTER
						l_nbr.background_color = (1,1,1,0.8)	# délavé
						ol_nbr = ObjCInstance(l_nbr)
						bo.addSubview_(ol_nbr)	
...
def long_press_action(sender):
	...
	for annotation in sender.mk_map_view.annotations():
		if str(annotation.title()) == 'Dropped Pin':
			lat = annotation.coordinate().a
			lon = annotation.coordinate().b
			sender.remove_one_pin(annotation)
		...
		with open(path,mode='rt') as f:
			...
			for pt in locs.keys():
				...
				if locs[pt] == 'user':
					...
					subtit = ''
					for latg_long in locs[pt]:
						if isinstance(latg_long,tuple):
							latg,long = latg_long
							if subtit:
								subtit += '\n'
							subtit += str((latg,long))
							v.addCircleToMap(latg,long,close_distance,str((latg,long)))
							v.addCircleToMap(latg,long,0.3,str((latg,long)))
					if isinstance(locs[pt][-1],tuple):
						# old version file does not contain "number of needed"
						locs[pt].append(len(locs[pt]))
