...
class MyView(ui.View):
	...
	def dimensions(self):				
		...
		x = x0 + 3 * (d + dd)
		y = h_icons + dd + 1 * (d + dd)
		def b_insert_action(sender):
			t = sender.title
			keyboard.insert_text(t)
		b = self.make_button(x,y,d,'A',self.typeChar)
		def long_press_handler(data):
			#print('long_press')
			b = data.view
			xp,yp = data.location
			xp,yp = ui.convert_point(point=(xp,yp), from_view=b, to_view=self)
			#print(b.name)
			if data.state == 1:
				# start long press
		...
		yy = y
		xx = x - (d+dd)
		b = self.make_button(xx,yy,d,'B',None)
		b.hidden = True
		yy = y
		xx = x + (d+dd)
		b = self.make_button(xx,yy,d,'C',None)
		b.hidden = True
		xx = x 
		yy = y - (d+dd)
		b = self.make_button(xx,yy,d,'D',None)
		b.hidden = True
		xx = x 
		yy = y + (d+dd)
		b = self.make_button(xx,yy,d,'E',None)
		b.hidden = True
		# flick button: end ===================================================
