'''
todo
	- key for uppercase of only one next letter
	- key for uppercase all next letters until retapped
...
# bugs
# - some emojis are not displayed (long unicodes?)
...
class MyView(ui.View):
	...
	def dimensions(self):				
		...
		keyboards = {'japan':[
			...
			[1,0,'あ',self.typeChar,'いうえお'],			
			[2,0,'か',self.typeChar,'きくけこ'],			
			[3,0,'さ',self.typeChar,'しすせそ'],
			[4,0,'文末',self.b_bottom_action,''],	# end of sentence
			
			[0,1,'⬅️',self.b_left_action,''],
			[1,1,'た',self.typeChar,'ちつてと'],			
			[2,1,'な',self.typeChar,'にぬねの'],			
			[3,1,'は',self.typeChar,'ひふへほ'],		
			[4,1,'➡️',self.b_right_action,''],	
			
			[0,2,'⬆️',self.b_up_action,''],
			[1,2,'ま',self.typeChar,'みむめも'],			
			[2,2,'や',self.typeChar,'「ゆ」よ'],			
			[3,2,'ら',self.typeChar,'りるれろ'],			
			[4,2,'⬇️',self.b_down_action,''],
			
			[0,3,'copy' if keyboard.has_full_access() else 'no full' ,self.b_copy_action,''],		
			[1,3,'read to\ncursor',self.b_read_to_cursor_action,''],
			[2,3,'わ',self.typeChar,'をんー '],
			[3,3,'read\nall',self.b_read_all_action,''],
			[4,3,'左削除',self.b_delete_action,''],		
			
			[-1,1,'🔤', self.b_alpha_action,''],
			[-1,2,'🔢', self.b_digit_action,''],
