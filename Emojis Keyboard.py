...
modifications
- when a flick key or its of its sub-keys become blue, it is read aloud

correction of bugs
- 

[https://raw.githubusercontent.com/cvpe/Pythonista-scripts/master/Emojis%20Keyboard.py](https://raw.githubusercontent.com/cvpe/Pythonista-scripts/master/Emojis%20Keyboard.py)
  
todo
	- ask:no use of sentences thus no use of round-arrow button
		use of ✅ button?
		what do you think about: say english traduction of a unique kanji
		to differentiate "葉" and "歯"
		
	- As for the cursor move button, I want you to move to the beginning and the end 	
		of the whole sentence even if you start a new line.
...
class MyView(ui.View):
	...
	def b_hirgana_action(self, sender):
		self.hide_all()
		self.v_hirgana.hidden = False
	...
	def dimensions(self):				
		...
		self.v_hirgana = ui.View()
		self.v_hirgana.background_color = self.background_color
		self.v_hirgana.frame = (0,dyk,w,h)
		self.v_hirgana.hidden = False
		self.add_subview(self.v_hirgana)
		...
		self.v_digit.frame = self.v_hirgana.frame
		...
		self.vs = {'hirgana':self.v_hirgana, 'alpha':self.v_alpha, 'digit':self.v_digit, 'emoji':self.v_emoji, 'katakana':self.v_katakana}
	...
	def set_temp_kanji_lbl(self,t):
		...
		self.v_hirgana['kanji_button'].enabled = (self.temp_kanji != '')
	...
	def tableview_other(self,sender):		
		...
		url = 'https://kanjiapi.dev/v1/kanji/' + kanji[0]
