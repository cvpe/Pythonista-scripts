''' 
version 00.28 is available with
	  ...
	  if previous character is an Hiragana or a Katakana character,
	  	...
	  	if mark is the voiced mark, 
	  		if character has an existing specific character (ucode + 1)
	  			replace previous character by this specific one	  	 
- if use type a punctuation in the Hiragana keyboard and if temporary storage
  is still empty, the punctuation will be directly inserted in the TextView 
- after selection of a Kanji in the list (becomes red), tapping the "round arrow"
...
todo
	...
	- ask: remove all process of English sentences containing the Kanji 
	       in the Kanjis list
	- ask use ✅ button to accept the selected Kanji
	- ask: if you leave a flicking key without selecting a blue sub-key,
		do you want to read out a particular message like 
		"no key has been selected" or "キーが選択されていません"
...
class MyView(ui.View):
	...
	def dimensions(self):				
		...
		keyboards = {'hiragana':[
			...
			[2,0,'文頭',self.b_top_action,''], # start of sentence
