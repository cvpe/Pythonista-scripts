...
todo	
	- ask: gray kanji key if no kanki would be converted to
	...
	- Please adjust the flick input button finely.
		...
		I can't type when I move it enough to get off the button.
		...
		for kbd in keyboards.keys():		
			for ix,iy,t,act,flick in keyboards[kbd]:
				...
				if act == self.b_kanji_action:
					...
					self.temp_kanji_lbl.frame = (self.dd,self.dd,self.width-2*self.dd,self.dy)
					self.temp_kanji_lbl.font = ('Arial',self.dy)
