''' 
version **00.43** is available with

modifications
  - alphabet page: exchange positions of buttons 絵文字 <--> 平仮名
  - small key enabled only if temporary storage contains 
    only letters having a small version
  - tapping small key converts temporary storage letters to their small version  
  - no more small keys page
...
class MyView(ui.View):
	...
	def set_temp_kanji_lbl(self,t):
		...
		# check if temporary storage contains only letters having small version
		kbd = None
		...
		if not self.v_katakana.hidden:
			...
			v = self.v_katakana
		if kbd:	
			only_small = (t != '') # we need at least one letter	
			for c in t:
				if c not in self.small_keys[kbd]:
					only_small = False
					break
			v['smallkey_button'].enabled = only_small
	...
	def smallKey(self, sender):
		...
		if not self.v_katakana.hidden:
			kbd = 'katakana'	
		s = ''
		for c in self.temp_kanji_lbl.text:			
			s += self.small_keys[kbd][c]
		self.set_temp_kanji_lbl(s)
