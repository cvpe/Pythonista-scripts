''' 
version **00.47** is available with

modifications
  - for trying: flicked sub-keys magnified, instead of 1.3,
    of 1.0 for あ
    of 1.1 for か
    of 1.2 for さ
...
todo
	...
	- I would like you to change the offset position of the "あ, か, さ" button 
		on the hiragana page, "ア, カ, サ" katakana page.
		I want you to put it a little higher.
	- It's okay to hide the letters on the top button a little.
		My wife is going to type "あ", but she often ends up typing "う".
		If it is an instant tap, it will happen with high probability.
  ...
  - ask: as her wife dors not see the temporary storage, we could not show it, 
  	then we could have 4 bigger rows using the whole authorized area.
  	Want to try?
	...
	def dimensions(self):				
		...
		for kbd in keyboards.keys():
			for ix,iy,t,act,flick in keyboards[kbd]:
				...
				if flick:
					long_press(b,self.long_press_handler, minimum_press_duration=0.1)
