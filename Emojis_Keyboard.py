''' 
version **00.55** is available with

modifications
  - when tapping Katakana key "カタカナ", convert' if possible, Hirgana letters of
    temporary storage in their Katakana letter and move temporary storage to
    TextView
...
todo
	...
	- Can I change the function of the "カタカナ" button to that of the Katakana
		conversion button?

		For example :

		Enter "あ"→ Insert into temporary storage.→ Press the "カタカナ" button.→ Convert to "ア".

		I want you to convert not only one character but all the characters in the temporary storage.
...
class MyView(ui.View):
	...
	def dimensions(self):				
		...
		k = 0
		for c in hirganas:
			self.hirgana_to_katakana[c] = katakanas[k]
			k += 1
