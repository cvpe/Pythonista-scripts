...
modifications
  - use parameter of long press to force to only check the finger move if this move 
    has been more than the an half key height, instead of rhe default of 10 pixels ,
    this oping a quicker process when selecting a sub-flicked key 
...
todo
	...
	- Can I display the selection field of the flick input button at the normal tap 
		speed instead of the long tap?

		The following characters can be entered immediately :.

		あ,か,さ,た,な,は,ま,や,ら,わ
		ア,カ,サ,タ,ナ,ハ,マ,ヤ,ラ,ワ

		I can input the first button right away, but the input speed of characters after flick selection is slow.
		Can I set the speed letters at the same speed as the normal button?

		With the normal flick input button of iPhone, you can input even if you move faster than the selection input field appears.

		My wife's finger movements are almost the same as those of a healthy person, so I'm a little troubled.
...
class MyView(ui.View):
	...
	def dimensions(self):				
		...
		keyboards = {'hiragana':[
			...
			[1,2,'カタカナ', self.b_katakana_action,''],
			...
			[0,1,'ABC', self.b_alpha_action,''],
			[0,2,'数字', self.b_digit_action,''],
			[5,3,'平仮名', self.b_hiragana_action,'']
