''' 
version 00.33 is available with

modifications
  - change the display of the "アルファベット" button to "ABC".
  - sound mark button display as "゛濁点"
    I need a first ゛character as first flicked key
    - “゛” read as ”濁点”
    - ”゜” read as ”半濁点”
  - display the punctuation button as "、。？！" 
    - "、" read as "てん"
    - "。" re	d as "まる"
  - add "left delete button" to digits sub-keyboard
  - add "left delete button" to alphabet sub-keyboard
  - change layout of digits sub-keyboard and add +,−,×,÷,= symbols
      1,2,3,4,5
      6,7,8,9,0
      +,−,×,÷,=
  + drastic layout change of the Hiragana keyboard.
      “⬅️,文頭,read to cursor ,定型文read all,文末,➡️”
      “ABC,あ,か,さ,た,な,左削除”
      “数字,は,ま,や,ら,わ,漢字”
      “絵文字,小文字,カタカナ,記号,、。,゛゜,return”
  - add "Symbol" button and its symbols sub-keyboard, 
      〜ー•←→↑↓●○◎△▲□■
      without ⏩ and ⏪ buttons (not sure I correctly understood)
  - add "fixed phrases" button
      without any action actually
...
todo
	...
	- I would like you to make it possible to display multiple pages at once. 
		Please use a rectangular button or something like that. I would like you to change the page for the part that cannot be displayed like the button of "⏩".

	- I would like you to create a new "fixed phrase" button.

		List of common phrase buttons.

			“おはよう😊” 			“おはようございます☀️” 			“こんちには✨” 			“こんばんは🌙” 			“お疲れ様です🙇”
...
class MyView(ui.View):
	...
	def dimensions(self):				
		...
		self.v_symbol.frame = self.frame
		self.v_symbol.hidden = True
		self.add_subview(self.v_symbol)

		self.vs = {'hiragana':self.v_hiragana, 'alpha':self.v_alpha, 'digit':self.v_digit, 'emoji':self.v_emoji, 'katakana':self.v_katakana, 'symbol':self.v_symbol}
		...
		'''	
		keyboards = {'hiragana':[
			[0,0,'⬅️',self.b_left_action,''],
			[1,0,'文頭',self.b_up_action,''],		# begin of line	
			[2,0,'read to\ncursor',self.b_read_to_cursor_action,''],	
			[3,0,'定型文',self.b_fixed_phrases_action,''],
			[4,0,'read\nall',self.b_read_all_action,''],
			[5,0,'文末',self.b_down_action,''],	# end of line
			[6,0,'➡️',self.b_right_action,''],	
			...
			[2,3,'カタカナ', self.b_katakana_action,''],
			[3,3,'記号', self.b_symbol_action,''],
			[4,3,"。、",self.typeChar,"、？！ "], 
			[5,3,u'\u309B'+'濁点',self.typeChar,u'\u309C    '],	
	...
	def typeChar(self,sender):
		...
		if not self.v_hiragana.hidden:
			...
			# Katakana, Alphabet, Emoji, Symbol: direct insert in text
			keyboard.insert_text(t)
