''' 
version **00.44** is available with

modifications
  - the 小文字 key will be enabled only if the last letter of the temporary storage
    has a small version, and tapping it will only convert this last letter
...
todo
	...
	- if tap on a key of first row, I get the up flicked key
		debug or do not allow tap. In reality, it is not a tap but a short long press and the finger is on the up key.
