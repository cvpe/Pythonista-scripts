...
# ==========
# - japanese speech
# - move a dot separetely, not all the six together
#		- how to decide we move it?
#		- modify/save sttings
# - ask #shinya.ta if he prefers the 6 fingers togehther?
# - ask @shinya.ta if he prefers horizontal scrollview with Kanjis as buttons
...
class BrailleKeyboardInputAccessoryViewForTextField(ui.View):
	...
	def dimensions(self):		
		...
		dakuten_dots = ui.ImageView(name='dakuten_dots')
		dakuten_dots.frame = (100,0,wd,h)
		dakuten_dots.hidden = True
		self.add_subview(dakuten_dots)
		self.dots_xy = [(e,e),(e,e+(d+e)),(e,e+2*(d+e)),(14,e),(14,e+(d+e)),(14,e+2*(d+e))]
	...
	def delete_button_action(self,sender):		
		if self['hirganas']['hirgana'].width > 0:
			...
			self.draw_hirganas()
			# of Kanji selection was in progress, cancel it
	...
	def dots_touched(self):
		...
		if self.dakuten != '':
			self.draw_dots(self['dakuten_dots'],self.dakuten)			
			dakuten_key = self.dakuten + '|' + self.seq
		else:
			dakuten_key = self.seq

		if dakuten_key not in self.Japanese_Braille:
			temp_hirgana = '?'
		else:
			temp_hirgana = self.Japanese_Braille[dakuten_key]
		...
		if self.dakuten != '':
			self['dakuten_dots'].x = self['dots'].x - self['dakuten_dots'].width
			self['dakuten_dots'].y = self['dots'].y
	...
	def key_pressed(self,key):
		...
		if self.dakuten == '':
			...
			dakuten_key = self.dakuten + '|' + key
		if dakuten_key in self.Japanese_Braille:
			ch = self.Japanese_Braille[dakuten_key]
			self.dakuten = ''
			if ch =='_':
				self.dakuten = key  		# store dot of dakuten
				self['dakuten_dots'].image = self['dots'].image
				self['dakuten_dots'].x = self['dots'].x - self['dakuten_dots'].width	# move it left
				self['dakuten_dots'].y = self['dots'].y
				self['dakuten_dots'].hidden = False
				return
