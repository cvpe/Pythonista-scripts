...
#		- touch identified by touch_id AND dot n°
#		- new touch in same dot, keep last only
# - 6 fingers for dots placement
# - read/save/ settings
#		- if not in keychain, set with computed values
...
class BrailleKeyboardInputAccessoryViewForTextField(ui.View):
	...
	def dimensions(self):		
		...
		if settings_str:
			...
			self.settings = {
				'portrait':{'1':(100,(1+(1-1))*diam),'2':(100,(1+(3-1))*diam),'3':(100,(1+(5-1))*diam),'4':(500,(1+(1-1))*diam),'5':(500,(1+(3-1))*diam),'6':(500,(1+(5-1))*diam)},
