...
class BrailleKeyboardInputAccessoryViewForTextField(ui.View):
	...
	def tableview_other(self,sender):														# add 1.6
		self['kanjis_ok'].title = self.select_text + self.get_kanji(0,next_sentence=True)											# add 1.6
	...
	def dots_touched(self):
		...
		if self.prefix != '':
			...
			if prefix_key not in self.Japanese_Braille:
				...
				#self['prefix_dots'].hidden = True
				if self.prefix == '3456':											# add 1.7
					prefix_key = self.seq
		else:
			prefix_key = self.seq

		if prefix_key not in self.Japanese_Braille:
			if self.prefix == '5':																			# add 1.7
				# special process for dot-5																# add 1.7
				# could be a prefix for dakuten														# add 1.7
				# or an interpoint char if alone													# add 1.7
				# at this moment, the second char is not yet certain			# add 1.7
				if self.seq not in self.Japanese_Braille:									# add 1.7
					temp_hirgana = self.Japanese_Braille[self.prefix] +	'?'	# add 1.7
				else:																											# add 1.7
					temp_hirgana = '・' +	self.Japanese_Braille[self.seq]		# add 1.7
					self['b_decision'].hidden = False												# add 1.7
			else:																												# add 1.7
				temp_hirgana = '?'
		else:
	...
	def key_pressed(self,key):
		...
		if prefix_key in self.Japanese_Braille:
			...
			if self.prefix == '5':																			# add 1.7
				...
				# or an interpoint char if alone													# add 1.7
				# at this moment, the second char is not yet certain			# add 1.7
				if key in self.Japanese_Braille:													# add 1.7
					ch = '・' +	self.Japanese_Braille[key]									# add 1.7
		if ch != '':																									# add 1.7
