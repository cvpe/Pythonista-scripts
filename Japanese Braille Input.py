...
#				 ***** meeds to be confirmed by user tests *****
...
"""
questions
- the Braille dots set with only dot 5 can be used as
		- a prefix for Dakuon
		- an interword dot
	do you agree if the program, when it meets a dot-5 character,
	it will consider it as a prefix unless if next character is not
	a dakuon dots set?
- when you tap on up/down/ok button of the conversion list, VoiceOver
	says a sentence as an example of the Kanji.
	Do you want a supplementar button with as title another sentence,
	so, if tapped, VoiceOver will say another example?
- buttons have an icon and a title used by VoiceOver.
...
#				 if character after dot-5 is not a character waitd after the prefix,
#						it will be processd as a dot-5 point
#				 ========== ASK CONFIRMATION ====================
# - new: conversion list elements, sentence
#					- button? try another sentence, on which button?
#				 ========== ASK CONFIRMATION ====================
# - mod: use transparent color for buttons titles if an image also exist.
#				so, only the icon is visible but VoiceOver still says the title
#				 ========== ASK CONFIRMATION ====================
...
class BrailleKeyboardInputAccessoryViewForTextField(ui.View):
	def __init__(self, frame=None,*args, **kwargs):
		...
		'235':'！',			#
		'56':'、',				# comma
		#'5':'・'	,			# inter words point, quid vs prefix of dakuten?
	...
	def key_pressed(self,key):
		...
		if prefix_key in self.Japanese_Braille:
			...
			if ch =='_':
				...
				if not self.singular_quotation_marks:
					ch = ch[0]
					self.singular_quotation_marks = True
				else: 
					ch = ch[1]
					self.singular_quotation_marks = False
